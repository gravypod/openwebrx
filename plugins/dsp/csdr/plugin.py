...
class dsp_plugin:
    ...
    def chain(self, which):
        ...
        any_chain_base += self.format_conversion + (
        " | " if self.format_conversion != "" else "")  ##"csdr flowcontrol {flowcontrol} auto 1.5 10 | "
        if which == "fft":
        ...
        if which == "nfm":
            return chain_begin + "csdr fmdemod_quadri_cf | csdr limit_ff | csdr fractional_decimator_ff {last_decimation} | csdr deemphasis_nfm_ff 11025 | csdr fastagc_ff 1024 | csdr convert_f_s16" + chain_end
        elif which == "am":
            return chain_begin + "csdr amdemod_cf | csdr fastdcblock_ff | csdr fractional_decimator_ff {last_decimation} | csdr agc_ff | csdr limit_ff | csdr convert_f_s16" + chain_end
        elif which == "ssb":
            return chain_begin + "csdr realpart_cf | csdr fractional_decimator_ff {last_decimation} | csdr agc_ff | csdr clipdetect_ff | csdr limit_ff | csdr convert_f_s16" + chain_end
    ...
    def start(self):
        ...
        command = command_base.format(bpf_pipe=self.bpf_pipe, shift_pipe=self.shift_pipe, decimation=self.decimation, \
                                      last_decimation=self.last_decimation, fft_size=self.fft_size,
                                      ...
                                      start_bufsize=self.base_bufsize * self.decimation, nc_port=self.nc_port, \
    ...
    def stop(self):
        ...
        if self.shift_pipe:
            try:
                ...
                print("[openwebrx-dsp-plugin:csdr] stop() :: unlink failed: " + self.shift_pipe)
        if self.squelch_pipe:
            try:
                os.unlink(self.squelch_pipe)
            except:
                print("[openwebrx-dsp-plugin:csdr] stop() :: unlink failed: " + self.squelch_pipe)
        if self.smeter_pipe:
            try:
                os.unlink(self.smeter_pipe)
            except:
                print("[openwebrx-dsp-plugin:csdr] stop() :: unlink failed: " + self.smeter_pipe)
        self.running = False
