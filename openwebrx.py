...
def main():
    ...
    # Open log files
    logs = type("logs_class", (object,),
                {"access_log": open(cfg.access_log if cfg.access_log else "/dev/null", "a"), "error_log": ""})()

    # Set signal handler
    signal.signal(signal.SIGINT,
                  handle_signal)  # http://stackoverflow.com/questions/1112343/how-do-i-capture-sigint-in-python
    ...
    # Pypy
    if pypy: print(
        "pypy detected (and now something completely different: a c code is expected to run at a speed of 3*10^8 m/s?)")
    ...
    if os.system("ncat --version > /dev/null") == 32512:  # check for ncat
        print(
            "[openwebrx-main] Error: ncat not detected, please install it! The ncat tool is a netcat alternative, used for distributing the I/Q data stream. It is usually available in the nmap package (sudo apt-get install nmap). For more explanation, look into the README.md")
        return
...
def close_client(i, use_mutex=True):
    ...
    access_log("Stopped streaming to client: " + clients[i].ip + "#" + str(clients[i].id) + " (users now: " + str(
        len(clients) - 1) + ")")
    del clients[i]
    if use_mutex: cmr()
...
class WebRXHandler(BaseHTTPRequestHandler):
    ...
    def send_302(self, what):
        ...
        self.end_headers()
        self.wfile.write(
            "<html><body><h1>Object moved</h1>Please <a href=\"/{0}\">click here</a> to continue.</body></html>".format(
                what))
        ...
        try:
            ...
            if self.path[:4] == "/ws/":
                ...
                try:
                    ...
                    rxws.send(self,
                              "MSG center_freq={0} bandwidth={1} fft_size={2} fft_fps={3} audio_compression={4} fft_compression={5} max_clients={6} setup".format(
                                  str(cfg.shown_center_freq), str(cfg.samp_rate), cfg.fft_size, cfg.fft_fps,
                                  cfg.audio_compression, cfg.fft_compression, cfg.max_clients))
                    ...
                    access_log("Started streaming to client: " + self.client_address[
                        0] + "#" + myclient.id + " (users now: " + str(len(clients)) + ")")
                ...
                if extension == "wrx" and (checkresult or receiver_failed):
                    ...
                    return
                anyStringsPresentInUserAgent = lambda a: reduce(lambda x, y: x or y,
                                                                [self.headers['user-agent'].count(b) for b in a], False)
                if extension == "wrx" and ((not anyStringsPresentInUserAgent(
                        ("Chrome", "Firefox", "Googlebot", "iPhone", "iPad", "iPod"))) if 'user-agent' in list(
                        self.headers.keys()) else True) and (not request_param.count("unsupported")):
