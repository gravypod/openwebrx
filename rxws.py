...
def handshake(myself):
    ...
    if (not h_key_exists("upgrade")) or not (h_value("upgrade") == "websocket") or (
    not h_key_exists("sec-websocket-key")):
...
def get_header(size):
    ...
    if (size > 125):
        ...
        extended_size = chr((size >> 8) & 0xff) + chr(
            size & 0xff)  # Okay, it uses reverse byte order (little-endian) compared to anything else sent on TCP
    else:
        ws_second_byte = size
...
def send(myself, data, begin_id="", debug=0):
    ...
    while True:
        ...
        if from_end + len(begin_id) > base_frame_size:
            ...
            flush(myself)
            if debug: print(
                "rxws.send ==================== #1 if branch :: from={0} to={1} dlen={2} hlen={3}".format(counter,
                                                                                                          counter + base_frame_size - len(
                                                                                                              begin_id),
                                                                                                          len(
                                                                                                              data_to_send),
                                                                                                          len(header)))
